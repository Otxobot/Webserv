...
#include "Server.hpp"
#include "../response/Response.hpp" // Agrega la ruta correcta al archivo de encabezado de Response.hpp
...
void Server::createSocket()
{
    ...
    if (setsockopt(_masterSockFD, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(int)) == -1)
        throw std::runtime_error("Unable to set socket option to the socket");
}
